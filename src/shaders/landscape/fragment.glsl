...
// The texture.
uniform sampler2DArray[16] u_textures;
...
void main() {
  ...
  if (v_texture == 13.0) texel = texture(u_textures[13], vec3(v_texcoord, layer));
  if (v_texture == 14.0) texel = texture(u_textures[14], vec3(v_texcoord, layer));
  if (v_texture == 15.0) texel = texture(u_textures[15], vec3(v_texcoord, layer));
